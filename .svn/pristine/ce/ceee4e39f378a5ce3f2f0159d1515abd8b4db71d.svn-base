...
@WebServlet("/mangerSellInfo.do")

public class ManagerSellInfoController extends HttpServlet {
...
//    }
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		System.out.println("관리자 출품 관리 페이지");
		HttpSession session = req.getSession();
		session.setAttribute("mangerPageCode", "mangerSellInfo");
		...
		// 서비스 객체 얻기
		IManagerProdService service = ManagerProdServiceImpl.getInstance();
		// 전체 글 갯수 조회하기
		int totalCount = service.countSell();
...
//		}
		...
		// 범위 별 게시물 조회
		List<BuyVo> list = service.selectSell(map);
