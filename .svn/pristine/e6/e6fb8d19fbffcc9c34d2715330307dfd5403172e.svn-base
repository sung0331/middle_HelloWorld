...
@WebServlet("/managerDepartmentInfo.do")

public class ManagerDepartmentInfoController extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		System.out.println("관리자 직원 조직도 페이지");
		...
		// 서비스 객체 얻기
		IManagerEmpService service=ManagerEmpServiceImpl.getInstance();
		// 전체 글 갯수 조회하기
		int totalCount = service.countEmp();
...
//		}
		...
		// 범위 별 게시물 조회
		List<empVo> list = service.selectDepartment(map);
		req.setAttribute("list", list);
		List<DepartmentVo> department= service.departmentList();
