...
@WebServlet("/mangerCusInfo.do")

public class ManagerCusInfoController extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		System.out.println("관리자 회원 관리 페이지");
		HttpSession session = req.getSession();
		System.out.println(session);
		session.setAttribute("mangerPageCode", "mangerCusInfo");
		...
		// 서비스 객체 얻기
		IManagerCustomerService service = ManagerCustomerServiceImpl.getInstance();
		// 전체 글 갯수 조회하기
		int totalCount = service.countCustomer();
...
//		}
		...
		// 범위 별 게시물 조회
		List<CustomerVo> list = service.selectCustomer(map);
