...
@WebServlet("/managerProductInfo.do")

public class ManagerProductInfoController extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		System.out.println("관리자 상품 관리 페이지");
		HttpSession session = req.getSession();
		System.out.println(session);
		session.setAttribute("mangerPageCode", "managerProductInfo");
		...
		// 전체 글 갯수 조회하기
		int totalCount = service.countProd();
...
//		}
		...
		// 범위 별 게시물 조회
		List<ProductVo> list = service.selectProd(map);
