...
@WebServlet("/mypageReviews.do")
public class MypageReviewsController extends HttpServlet{
    
    IMyPageBoardService myPageBoardService = MyPageBoardServiceImpl.getInstance(); 
    
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("내가 쓴 리뷰 페이지");
        HttpSession session = req.getSession();
        Integer userNo = (Integer)session.getAttribute("userNo");
        session.setAttribute("myPageCode", "mypageReviews");
        ...
        // 전체 게시글 개수 가져오기
        int totalCount = myPageBoardService.countMyReviews(userNo);
        ...
        // 사용자 게시물 가져오기
        List<BoardVo> list = myPageBoardService.selectReviewsPaging(map);
        ...
        // 데이터가 올바르게 전달되는지 로그 확인
        for(BoardVo board : list) {
            System.out.println("게시물 번호: " + board.getPost_no() + ", 작성 날짜: " + board.getBoard_date());
        }
